...
class CommentsTable extends Table
{
    ...
    public function validationDefault(Validator $validator)
    {
        $validator
            ->integer('comment_id')
            ->allowEmptyString('comment_id', 'create');

        $validator
            ->dateTime('comment_date')
            ->requirePresence('comment_date', 'create')
            ->allowEmptyDateTime('comment_date', false);

        $validator
            ->scalar('comment_desc')
            ->maxLength('comment_desc', 1000)
            ->requirePresence('comment_desc', 'create')
            ->allowEmptyString('comment_desc', false);
...
//    {
//        $rules->add($rules->existsIn(['task_id'], 'Projects'));
//
//
//        return $rules;
//    }
