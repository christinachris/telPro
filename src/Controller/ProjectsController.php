...
// Auth
        ...
        elseif($user_role =='Admin'|| 'Superadmin'){
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            if ($this->request->getData()['Progress_Status'] == 1) {
                ...
                $projects = $this->Projects->find('all')->where(['AND' => ['Projects.archive' => 0]])->contain(['Clients','Talents']);
            }
        }
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if($this->request->getData()['end_date']!=''&&$this->request->getData()['start_date']) {
                if (date('Y-m-d H:i:s',strtotime(strtr($this->request->getData()['end_date'],'-', ' '))) < date('Y-m-d H:i:s',strtotime(strtr($this->request->getData()['start_date'],'-', ' ')))) {
            ...
            if($errorMessage==false) {
				...
				if($this->request->getData()['start_date']!=''){
					...
					$project->start_date = date('Y-m-d H:i:s',strtotime(strtr($this->request->getData()['start_date'],'-', ' ')));
                ...
                if($this->request->getData()['end_date']!='') {
					...
					$project->end_date = date('Y-m-d H:i:s',strtotime(strtr($this->request->getData()['end_date'],'-', ' ')));
                }
                else{
    ...
    public function edit($id = null)
    {
        ...
        $errorMessage=false;
        if ($this->request->is(['patch', 'post', 'put'])) {
			//$end_date1=date('Y-m-d H:i:s',strtotime($this->request->getData()['end_date']));
			//$start_date1=date('Y-m-d H:i:s',strtotime($this->request->getData()['start_date']));
            if($this->request->getData()['talent_name']==''){
                $this->set('errors','please choose at least one talent.');
                $errorMessage=true;
            }
            ...
            if($errorMessage==false) {
				...
				if($this->request->getData()['start_date']!=''){
                    //$project->start_date = $this->request->getData()['start_date'];
					$project->start_date = date('Y-m-d H:i:s',strtotime(strtr($this->request->getData()['start_date'],'-', ' ')));
					//$sd= date_create_from_format("d-M-Y g:i A",$this->request->getData()['start_date']);
					//$project->start_date = date("Y-m-d h:i:s", strtotime($sd));
					
					//$project->start_date = date('Y-m-d H:i:s',strtotime($start_date1));
                }
                else{
                    $project->start_date = null;
                }
				...
				if($this->request->getData()['end_date']!='') {
                    //$project->end_date = $this->request->getData()['end_date'];
					$project->end_date = date('Y-m-d H:i:s',strtotime(strtr($this->request->getData()['end_date'],'-', ' ')));
					//$project->end_date = date('Y-m-d H:i:s',strtotime($this->request->getData()['end_date']));
					//$project->end_date = date('Y-m-d H:i:s',strtotime($end_date1));
                }
                else{
                    $project->end_date = null;
                }
    ...
    public function delete($id = null)
    {
        ...
        $mentions=$this->Mentions->find('all',['conditions'=>['project_id'=>$id]])->toList();
        for($i=0;$i<sizeof($mentions);$i++){
            $this->Mentions->delete($mentions[$i]);
        }
